    ...
    return Q.try(function() {
        return self.envelope(data).then(function(data) {
            return self.compress(data).then(function(resp) {
                ...
                if (resp.stream) {
                    resp.stream.pipe(response).on("error", function(error) {
                        throw new DataError({ message: "Failed to stream resonse.", data: { response: response }});
                    });
                }
                else {
                    if (resp.content) {
                        try {
                            ...
                            throw new DataError({ message: "Failed to write resonse.", data: { response: response }});
                        }
                    }
                    response.end();
                }
