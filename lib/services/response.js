...
class ResponseService {
    ...
    envelope(data) {
        ...
        if (data.header["Content-Type"]) {
            if (data.header["Content-Type"].match(/\bapplication\/json\b/)) {
                if (data.content && !(data.content instanceof String || typeof data.content === "string")) {
                    try {
    ...
    compress(data, reject) {
        try {
            if (data.fresh) return Promise.resolve({ header: data.header, statusCode: 304 });    
            else if (data.header["Content-Encoding"] == "gzip") return this.compressGzip(data, reject);
            else if (data.header["Content-Encoding"] == "deflate") return this.compressDeflate(data, reject);
    ...
    send(response, data) {
        return new Promise((resolve, reject) => {
            try {
                this.envelope(data);
                return this.compress(data, reject).then(resp => {
                    try {
