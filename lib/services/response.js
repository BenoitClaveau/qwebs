...
ResponseService.prototype.envelope = function (data) {
    ...
    return Q.try(function () {
        ...
        data.header["Date"] = data.header["Date"] || new Date(atime).toUTCString();
        data.header["Expires"] = data.header["Expires"] || new Date(atime).toUTCString();
        ...
        if (!data.fresh) data.header["Last-Modified"] = atime.toUTCString();
