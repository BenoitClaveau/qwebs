...
class ResponseService {
    ...
    _envelope(responsePromise, data) {
        try {
            ...
            if (data.header["Content-Type"]) {
                ...
                if (data.stream) data.header["Transfer-Encoding"] = data.header["Transfer-Encoding"] || "chunked";
                //TODO Content-Length
            }
        }
        catch(error) {
    ...
    send(response, data) {
        return new Promise((resolve, reject) => {
            ...
            if (response.headersSent) {         //Http header could be already sending for stream. Change statusCode here has no effect bevause header has already be sent
