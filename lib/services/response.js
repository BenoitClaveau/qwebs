        ...
        if (data.contentGzip) //assets already gzip
            return { header: data.header, content: data.contentGzip, statusCode: data.statusCode };
             ...
             var onerror = function(error) {
                throw new DataError({ message: "Failed to defalte content." });
            };
        ...
        if (data.header["Content-Type"].match(/\bapplication\/json\b/)) {
            if (!(data.content instanceof String)) {
                try {
                    ...
                    throw new DataError({ message: "Failed to stringify content", data: {content: data.content}});
                }
            }
        ...
        return self.compress(data).then(function(resp) {
            try {
                ...
                if (resp.stream) {
                    ...
                    var onerror = function(error) {
                        throw new DataError({ message: "Failed to stream resonse.", data: { status: resp.statusCode, header: resp.header }});
                    };
                    ...
                    if (resp.content) return Q.ninvoke(response, "end", resp.content);
                    else  Q.ninvoke(response, "end");
                }
            }
            catch(error) {
                throw new DataError({ message: "Failed to send resonse.", data: { status: resp.statusCode, header: resp.header }});
            }
        }).then(function() {
            return self;
        });
