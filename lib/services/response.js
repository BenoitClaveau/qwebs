...
class ResponseService {
            ...
            if (data.header["Content-Type"].match(/\bapplication\/json\b/)) {
                if (data.content && !(data.content instanceof String)) {
                    ...
                    data.stream = data.stream
                        .once("error", function(error) { //Use old syntax functio(error), 'this' will be the stream.
    ...
    compressGzip(data) {
        return new Promise((resolve, reject) => {
            if (data.contentGzip) { //assets already gzipped
                ...
                resolve({ header: data.header, stream: stream, statusCode: data.statusCode });
            }
    ...
    compressDeflate(data) {
        return new Promise((resolve, reject) => {
            if (data.contentDeflate) { //assets already deflated
                ...
                let stream = data.stream
                                .on("error", function(error) { //Use old syntax functio(error), 'this' will be the stream.
                                    this.emit("error", error);
                                })
    ...
    send(response, data) {
        return new Promise((resolve, reject) => {
            try {
                if (response.streamError) return; //because error has already been sent via stream error
                this.envelope(data);
                    ...
                    try {
                        ...
                        if (resp.stream) {
                            resp.stream
                                ...
                                .pipe(response)
                                .once("error", function(error) { //already sending. Only close the request
                                    resp.statusCode = error.statusCode || 500;
                                    resp.end();
                                    response.streamError = true; //flag response
                                    reject(error);
                                })
                                .once("finish", () => {
                                    resolve(resp);
                        ...
                        if (!(error instanceof DataError)) reject(new DataError({ statusCode: 500, request: data.request, message: error.message, stack: error.stack }));
