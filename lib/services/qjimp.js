...
class QJimp {
    ...
    clone(image) {
        return Promise.resolve().then(() => {
            if (!image) throw new WebError({ message: "image is not defined." });
            return image.clone();
        });
    };
    ...
    toBuffer(image, contentType) {
        return new Promise((resolve, reject) => {
            if (!contentType) reject(new WebError({ message: "contentType is not defined." }));
    ...
    resize(image, width, height) {
        return Promise.resolve().then(() => {  
            if (!image) throw new WebError({ message: "image is not defined." });
            if (!width) throw new WebError({ message: "width is not defined." });
            if (!height) throw new WebError({ message: "height is not defined." });
    ...
    crop(image, left, top, width, height) {
        return Promise.resolve().then(() => {  
            if (!image) throw new WebError({ message: "image is not defined." });
            if (left == null) throw new WebError({ message: "left is not defined." });
            if (top == null) throw new WebError({ message: "top is not defined." });
            if (width == null) throw new WebError({ message: "width is not defined." });
            if (height == null) throw new WebError({ message: "height is not defined." });
    ...
    cropAndResize(image, width, height) {
        return this.size(image).then(size => {
            if (!image) throw new WebError({ message: "image is not defined." });
            if (!size) throw new WebError({ message: "size is not defined." });
            if (!width) throw new WebError({ message: "width is not defined." });
            if (!height) throw new WebError({ message: "height is not defined." });
