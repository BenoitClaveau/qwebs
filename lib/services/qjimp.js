...
class QJimp {
    ...
    cropAndResize(image, size, width, height) {
        return Promise.resolve().then(() => {  
            ...
            if (cropRatio < imageRatio) {
                ...
                return this.resize(image, resizedWidth, height).then(function(resizedImage) {
                    return this.size(resizedImage).then(function(resized) {
                ...
                return this.resize(image, width, resizedHeight).then(function(resizedImage) {
                    return this.size(resizedImage).then(function(resized) {
    ...
    cropAndAddBackground(image, size, width, height) {
        return Promise.resolve().then(() => {  
            ...
            return this.cropIfNeeded(image, size, width, height).then(function(cropped) {
                return this.size(cropped).then(function(size) {
                    return this.clone(cropped).then(function(background) {
                        ...
                        return this.crop(background, bgLeft, bgTop, bgWidth, bgHeight).then(function(background) {                    
                            return this.resize(background, width, height).then(function(background) {
                                return this.blur(background, 100).then(function(background) {
                                    return this.opacity(background, 0.3);
                                });
                            });
                        }).then(function(background) {

                            return this.scale(cropped, 0.75).then(function(cropped) {

                                return this.size(cropped).then(function(thSize) {
    ...
    smartResize(image, width, height) {
        return Promise.resolve().then(() => {  
            ...
            return this.size(image).then(function(size) {
                if (size.height < height && size.width < width) return this.cropAndAddBackground(image, size, width, height);
                else return this.cropAndResize(image, size, width, height);
            });
        });
    };
