...
class Asset {
    ...
    initFromVirtualFile(file) {
        return Promise.resolve().then(() => {
            this.route = path.relative(file.base, file.path);
            let ext = path.extname(this.route);
            try {
                this.contentType = contentType.getFromExt(ext);
            }
    ...
    invoke(request, response) {
        return new Promise((reject, resolve) => {
            try {
                if (!request) throw new DataError({ request: request, message: "Request is undefined.", data: { route: this.route }});
                if (!response) throw new DataError({ request: request, message: "Response is undefined.", data: { route: this.route }});
                if (!request.url) throw new DataError({ request: request, message: "Request.url is undefined.", data: { route: this.route }});
                if (!this.content) throw new DataError({ request: request, message: "Content is empty", data: { route: this.route }});
