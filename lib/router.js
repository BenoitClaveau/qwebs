...
class Router {
    ...
    leaf(request) {
        switch (request.method) {
            ...
            default:
                throw new DataError({ statusCode: 404, request: request, message: "Method is unknown.", data: { method: request.method, pathname: request.pathname }});
        }
    };
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (!this.$qwebs.loaded) {
                    ...
                    throw new DataError({ statusCode: 503, request: request, message: "Server is loading.", data: { url: request.url }, header: header });
                }
                let leaf = this.leaf(request);
                if (!leaf) throw new DataError({ statusCode: 404, request: request, data: { method: request.method, pathname: request.pathname, query: request.query }});

                request.params = leaf.params;
                leaf.router.invoke(request, response).then(resolve).catch(reject);
            }
            catch(error) {
                reject(error);
            }
