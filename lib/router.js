...
class Router {
    ...
    leaf(request) {
        try {
            ...
            error.request = request; //extend error
            throw error;
        }
    };
    ...
    invoke(request, response, overridenUrl) {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (this.$proxy.match(request)) 
                    return this.$proxy.forward(request, response);
                ...
                if (leaf) {
                    ...
                    if (!this.$qwebs.loaded) {
                        ...
                        reject(new DataError({ statusCode: 503, request: request, message: "Server is loading.", data: { url: request.url }, headers: headers }));
                    }
                    
                    request.params = leaf.params;
                    leaf.router.invoke(request, response).then(resolve).catch(error => {
                        if (!(error instanceof DataError)) reject(new DataError({ statusCode: 500, request: request, message: error.message, stack: error.stack }));
                        if (!error.request) error.request = request;
                        reject(error);
                    });
                }
                else reject(new DataError({ statusCode: 404, request: request, data: { url: request.url }}));
            }
            catch(error) {
                if (!(error instanceof DataError)) reject(new DataError({ statusCode: 500, request: request, message: error.message, stack: error.stack }));
                if (!error.request) error.request = request;
