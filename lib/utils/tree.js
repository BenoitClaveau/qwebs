...
Nodes.prototype.push = function(node) {
	...
	if (!existingNode) {
        if (node.token.includes("*")) self._nodes.push(node);       //not prioritary
		else if (node.token.includes(":")) self._nodes.push(node);  //not prioritary. TODO find position of '*' and insert before his position
		else self._nodes.splice(-1, 0, node);                       //insert like first element
	}
	else {
		if(node.router) { //node could be a leaf
			if (existingNode.router) throw new DataError({ message: "Multiple end route.", data: { first: existingNode.router, second: node.router} });
...
Nodes.prototype.findOne = function(node) {
    ...
    //console.log("results", results.map(function(e) { return e.node.token }));
