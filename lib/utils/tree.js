...
Nodes.prototype.findOne = function(node, params) {
	...
	for(var i = 0; i < results.length; i++)
	{
		...
		if (!next) {
			if(result.node.router) {
				core.extend(result.params, params);
				return result;
			}
			else continue;
		}
		else {
			var item = result.node.nodes.findOne(next, params);
			if (item) {
				core.extend(item.params, params);
				return item;
			}
		}
	}
