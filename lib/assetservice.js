...
AssetService.prototype.load = function(qwebs, dir) {
    dir = dir || "./public";
    var self = this;
    ...
    return Q.nfcall(fs.stat, dir).then(function(stat) {
        ...
        for (var i in files) {
            ...
            var asset = self.add(new Asset(qwebs, route));
        ...
        throw new DataError({ message: "Failed to load public folder : " + dir, error: error });
    });
};
...
AssetService.prototype.invoke = function(request, response) {
    ...
    if (!asset.content) throw new DataError({ message: "Content is empty: " + asset.route });
    ...
    return response.send({
        ...
        throw new DataError({ message: "Failed to send: " + asset.content, error: error });
    });
};
