...
AssetService.prototype.add = function(asset) {
    ...
    //console.log("Asset inserted", this.list.length, asset.route);
    return asset;
}
...
AssetService.prototype.load = function(dir) {
    ...
    return Q.nfcall(fs.stat, dir).then(function(stat) {
        ...
        throw new DataError({ message: "Failed to load public folder.", error: error });
    });
};
...
AssetService.prototype.invoke = function(request, response) {
    ...
    if (!asset.content) throw new DataError({ message: "Content is empty: " + request.url });
