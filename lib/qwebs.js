...
class Qwebs {
    ...
    invoke(request, response, overriddenUrl) {
        return Promise.resolve().then(() => {
            ...
            request.pathname = request.part.pathname = decodeURI(request.part.pathname);
            request.part.query = request.part.query ? decodeURI(request.part.query) : null;
            request.query = request.part.query ? querystring.parse(request.part.query) : {};
