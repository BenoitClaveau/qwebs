...
class Qwebs {
    constructor(options) {
        ...
        this.inject("$initializer", "./services/initializer", { local: true });
    ...
    load() {
        return Promise.resolve().then(() => {
            ...
            let $initializer = this.resolve("$initializer");
            if (!$initializer) throw new DataError({ message: "$initializer is not defined." });

            return $assetsLoader.load().then(() => {
                return $bundleLoader.load().then(() => {
                    return $routesLoader.load().then(() => {
                        this.injector.load();
                        return $initializer.load().then(() => {
                            this.router.load();
                            this.loaded = true;
                            return this;
                        });
