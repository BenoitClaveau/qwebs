...
class Qwebs {
    constructor(options) {
        ...
        this.inject("$qwebsLoader", "./loaders/qwebs", { local: true });
        this.inject("$contentType", "./services/content-type", { local: true });
        ...
        this.inject("$streamProxy", "./services/stream-proxy", { local: true });
    };

    load() {
        return Promise.resolve().then(() => {
            let $qwebsLoader = this.resolve("$qwebsLoader");
