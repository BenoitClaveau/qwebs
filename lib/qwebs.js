...
Router.prototype.init = function(options) {
        ...
        return Q.try(function () {
            if (!options) throw new Error("option is not definied.");
            
            global.setConfig(options.config);
 
            return assets.load(global.config.folder).then(function () {  //load & create assets
                return self.createBundles(options);               //load and create bundle
            });
        });
};

Router.prototype.createBundles = function(options) {
        var self = this;
        return Q.try(function () {
            var promises = [];
...
Get.prototype.register = function(controller, viewName) {
    ...
    if (global.config.verbose) console.log(this.route, viewName);
    return this;
};
