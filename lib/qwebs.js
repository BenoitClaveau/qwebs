...
Qwebs.prototype.init = function(options) {
    options = options || {};
    var self = this;
    
    return configService.load(options.config).then(function(config) {
        self.config = config;
        return assetService.load(options.folder).then(function() {
            return bundleService.load(options.bundle).then(function(bundle) {
                var promises = [];
