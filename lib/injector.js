...
class Injector {
	...
	async load() {
		...
		await Promise.all(entries.map(async e => await e.service.import()));
		await Promise.all(entries.map(async e => await e.service.create()));
		await Promise.all(entries.map(async e => await e.service.mount()));
	};

	async unload() {
		const entries = this.entries; //create a copy
		await Promise.all(entries.map(async e => await e.service.unmount()));
	};
...
class Item {
	...
	async import() {
		...
		console.log("import", chalk.yellow(this.name), this.uuid);
	...
	async create() {
		...
		let args = []
		if (this.deps) 
			throw new Error("Cyclic reference for service ${service}.", { service: this.name, location: this.location, plan: this.injector.plan[this.name] });
		...
		console.log("create", chalk.green(this.name), this.uuid);
	...
	async mount() {		
		...
		if (this.instance && this.instance.mount) {
			console.log("mount", chalk.red(this.name), this.uuid);			
