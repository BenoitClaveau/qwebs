...
class Injector {
	...
	import() {
		...
		let services = Object.entries(this.container).map(([k, v]) => {
	...
	async resolve(name, options) {
		if (!name) throw new UndefinedError("Service", { name: name });
		options = options || {};
...
class Item {
	...
	create() {	
		...
		if (!this.description) throw new UndefinedError("Description of service ${service}", { service: this.name });
