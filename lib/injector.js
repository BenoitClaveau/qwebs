...
class Item {
	constructor($injector, name, service, options) {
		...
		if (typeof service == "object") {
			if (this.options.instanciate === false) throw new Error("Service cannot be an instance.", { name: this.name }});
	...
	load() {
		...
		if (this.location[0] == " ") throw new Error("Service location start with space.", { name: this.name }});
		let	object;
		...
		try {
			...
			throw new Error("Error on require.", { message: error.message,  name: this.name, location: this.location }, stack: error.stack });
		}		
	...
	create() {	
		...
		if (!this.description) throw new Error("Description of service is not defined.", { name: this.name }});
		...
		if (this.deps) throw new Error("Cyclic reference.", { name: this.name, plan: this.$injector.plan[this.name] }});
		...
		try {
			...
			throw new Error("No constructor found.", { name: this.name, fn: Function.prototype.toString.call(fn) }, stack: error.stack });
		}
		...
		if (!this.instance) throw new Error("Service cannot be created.", { name: this.name }});
	};
