...
class Item {
	...
	load() {
		...
		if (this.options.local) {
			try {
				...
				throw new DataError({ message: "Error on require.", data: { message: error.message, location: location }});
			}
		}
		else {
			...
			object = require.main.require(location);
				...
				throw new DataError({ message: "Error on require.", data: { message: error.message, location: this.location }});
			}
		}
	...
	create() {	
		...
		let tokens = [];
		try {
			tokens = Function.prototype.toString.call(fn)
						.replace(/{.*?}/g, '')
						.match(/constructor\s?\([\s\S]*?\)/)[0]
						.match(/^[^\(]*\(\s*([^\)]*)\)/m);
		}
		catch(error) {
			throw new DataError({ message: "No constructor found.", data: { name: this.name }});
		}
		...
		this.instance = new fn([null].concat(args))
