...
class Injector {
	...
	async unload() {
		const entries = this.entries; //create a copy
		await Promise.all(entries.map(e => e.service.unmount()));
	};
...
class Item {
	...
	async import() {
		...
		console.log(chalk.yellow("import"), this.name);
		...
		console.log(chalk.yellow("imported"), this.name);
	};

	async create() {
		console.log(chalk.green("create"), this.name);
	...
	async mount() {
		console.log(chalk.red("create"), this.name);
